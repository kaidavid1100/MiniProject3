...
public:
    ...
    void set(){
        int tmp = disc_count[1]-disc_count[2]+(corner(1)-corner(2))*100+(buffer(1)-buffer(2))*(-30)+(edge(1)-edge(2))*10+(danger_zone(1)-danger_zone(2))*(-10);
        if(cur_player == 1) value = tmp + (x_square(1)-x_square(2))*10;
        else if(cur_player == 2) value = tmp*(-1);
    ...
    int danger_zone(int type){
        int t=0;
        //if(next_board[1][1]==type) t++;
        if(next_board[1][2]==type) t++;
        if(next_board[1][3]==type) t++;
        if(next_board[1][4]==type) t++;
        if(next_board[1][5]==type) t++;
        //if(next_board[1][6]==type) t++;
        if(next_board[2][1]==type) t++;
        if(next_board[3][1]==type) t++;
        if(next_board[4][1]==type) t++;
        if(next_board[5][1]==type) t++;
        //if(next_board[6][1]==type) t++;
        if(next_board[2][6]==type) t++;
        if(next_board[3][6]==type) t++;
        if(next_board[4][6]==type) t++;
        if(next_board[5][6]==type) t++;
        //if(next_board[6][6]==type) t++;
        if(next_board[6][2]==type) t++;
        if(next_board[6][3]==type) t++;
        if(next_board[6][4]==type) t++;
        if(next_board[6][5]==type) t++;
        return t;
    }

    int x_square(int type){
        int t=0;
        if(next_board[2][2]==type) t++;
        if(next_board[3][3]==type) t++;
        if(next_board[4][4]==type) t++;
        if(next_board[5][5]==type) t++;
        if(next_board[5][2]==type) t++;
        if(next_board[4][3]==type) t++;
        if(next_board[3][4]==type) t++;
        if(next_board[2][5]==type) t++;
        return t;
    }
