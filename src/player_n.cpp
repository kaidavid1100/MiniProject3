...
public:
    ...
    State(std::array<std::array<int, 8>,8>& b, std::vector<Point>& n, int p){
        cur_player = p;
        for(int i=0;i<8;i++) for(int j=0;j<8;j++) next_board[i][j] = b[i][j];
        next_va = n;
    }
    ...
    void update(State next){
        ...
        for(Point dir:directions){
            ...
            p = p + dir;
            while(get_disc(p)!=0 && is_spot_on_board(p)){
        ...
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                ...
                if (is_spot_valid(p)) next.next_valids.push_back(n);
            }
        }
        next.cur_player = get_next_player(cur_player);
...
int alpha_beta(State cur, int depth, int alpha, int beta ,bool player_mode){
    if(cur.next_valids.size() == 0 || depth == 0) return cur.value;
    if(player_mode){
        int good=INT_MIN;
        for(State it:cur.next_valids){
            State nextstate=cur;
            nextstate.update(it);
            good=max(good,alpha_beta(it,depth-1,alpha,beta,false));
        ...
        for(State it:cur.next_valids){
            State nextstate=cur;
            nextstate.update(it);
            good=min(good,alpha_beta(it,depth-1,alpha,beta,true));
            beta=min(good,beta);
            if(alpha>=beta) break;
        }
        return good;
